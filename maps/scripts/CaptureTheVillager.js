Trigger.prototype.InitCaptureTheRelic = function()
{
	...
	let catafalqueTemplates = shuffleArray(cmpTemplateManager.FindAllTemplates(false).filter(
		name => GetIdentityClasses(cmpTemplateManager.GetTemplate(name).Identity || {}).indexOf("Villager") != -1));
...
Trigger.prototype.CheckCaptureTheRelicVictory = function(data)
{
	...
	if (!cmpIdentity || !cmpIdentity.HasClass("Villager") || data.from == INVALID_PLAYER)
		return;
	...
	if (data.to == -1)
	{
		warn("Villager entity " + data.entity + " has been destroyed");
...
Trigger.prototype.StartCaptureTheRelicCountdown = function(winningPlayers)
{
	...
	this.ownRelicsVictoryMessage = cmpGuiInterface.AddTimeNotification({
		"message": isTeam ?
			markForTranslation("%(_player_)s and their allies have captured all villagers and will win in %(time)s.") :
			markForTranslation("%(_player_)s has captured all villagers and will win in %(time)s."),
		"players": others,
		"parameters": {
	...
	this.othersRelicsVictoryMessage = cmpGuiInterface.AddTimeNotification({
		"message": isTeam ?
			markForTranslation("You and your allies have captured all villagers and will win in %(time)s.") :
			markForTranslation("You have captured all villagers and will win in %(time)s."),
...
Trigger.prototype.CaptureTheRelicVictorySetWinner = function(winningPlayers)
{
	...
	cmpEndGameManager.MarkPlayersAsWon(
		...
		n => markForPluralTranslation(
			"%(lastPlayer)s has won (Capture the Villager).",
			"%(players)s and %(lastPlayer)s have won (Capture theVillager).",
			n),
		n => markForPluralTranslation(
			"%(lastPlayer)s has been defeated (Capture the Villager).",
			"%(players)s and %(lastPlayer)s have been defeated (Capture the Villager).",
			n));
};

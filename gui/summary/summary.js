...
function init(data)
{
	...
	getGUIObjectByName("waterGatheredHeading").size = left + " 34 " + (left + width) + " 100%"; left += width;
	...
	getGUIObjectByName("exchangedWaterHeading").size = left + " 16 " + (left + width) + " 100%"; left += width;
	...
	for (var i = 0; i < MAX_SLOTS; ++i)
	{
		if (i < maxPlayers)
		{
			...
			var waterGathered = getGUIObjectByName("waterGathered["+i+"]");
			...
			var exchangedWater = getGUIObjectByName("exchangedWater["+i+"]");
			...
			waterGathered.size = left + " 2 " + (left + width) + " 100%"; left += width;
			...
			exchangedWater.size = left + " 2 " + (left + width) + " 100%"; left += width;
			...
			waterGathered.caption = SOLD_COLOR + playerState.statistics.resourcesGathered.water + "[/color] / " +
				BOUGHT_COLOR + (playerState.statistics.resourcesUsed.metal - playerState.statistics.resourcesSold.water) + "[/color]";
			...
			exchangedWater.caption = SOLD_COLOR + '+' + playerState.statistics.resourcesBought.water
				+ '[/color] ' + BOUGHT_COLOR + '-' + playerState.statistics.resourcesSold.water + '[/color]';
			var totalBought = 0;
